        ...
        def check_access(*args, **kws):
            ...
            if token:
                ...
                try:
                    ...
                    if check_permissions and 'role' in data and data['role'] == ROLE_USER and request.method != 'GET':
                        app.logger.info("Operation %s not permitted for user='%s/%s' from %s", request, data['name'], data['role'], request.remote_addr)
                        return jsonify({'error': 'operation not permitted'}), 403
                    return request_handler(*args, **kws)
                except jose.exceptions.JWTError:
                    app.logger.info("Bad token (%s) from %s", token, request.remote_addr)
                    return jsonify({'error': 'operation not permitted'}), 403
            else:
...
def authenticate():
    ...
    remote_address = request.remote_addr if request.remote_addr != "b''" else request.headers.get('X-Real-Ip')
    ...
    for user in User.query.all():
        if user.access_code == hash_access_code(request.json['access_code']):
            return jsonify({
                'user_token': jwt.encode({'name': user.name, 'role': user.role}, os.environ.get("SECRET"), algorithm='HS256'),
...
@authenticated()
def get_alert():
    alert = Alert.query.filter_by(end_time=None).order_by(Alert.start_time.desc()).first()
    if alert:
...
def sensors():
    if request.method == 'GET':
        ...
        if not request.args.get('alerting'):
            return jsonify([i.serialize for i in Sensor.query.filter_by(deleted=False).order_by(Sensor.channel.asc())])
        ...
        sensor = Sensor(channel=data['channel'], zone=zone, sensor_type=sensor_type, description=data['description'])
