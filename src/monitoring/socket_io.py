...
def start_socketio():
    ...
    app.run(threaded=False, host=os.environ['MONITOR_HOST'], port=int(os.environ['MONITOR_PORT']))
...
def connect(sid, environ):
    ...
    try:
        device_info = jwt.decode(qs['token'][0], os.environ.get('SECRET'), algorithms='HS256')
...
#             return False
        ...
        logger.error("Authentication failed from '%s'! token='%s'", remote_address, qs['token'][0])
        return False
